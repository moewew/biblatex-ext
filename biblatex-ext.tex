...
If at all possible, backwards compatiblity code that allows old code to
continue to work is in place. Sometimes that is not feasible and in those cases
the next best solution is to warn if old code is used, so please check the
log file for warnings --  especially after an update.
But at times not even that is possible, so it is always a good idea to check
the change history after an update.
