...
The package may load the support packages \hypersty{biblatex-ext-oa-doiapi} and
\hypersty{biblatex-ext-oa-tikzsymbols}.
...
Load the support package \hypersty{biblatex-ext-oa-tikzsymbols} with its
load-time option \opt{symbol} set to \prm{symbol identifier}.
...
\marginwarn Note that \hypersty{biblatex-ext-oa-tikzsymbols} will load \pgftikz
...
the macro \hypercmd{oasymbol} must be defined and should contain an open access
symbol that can be printed in the margin.
The package \sty{biblatex-ext-oa} itself does not provide a defintion for
\hypercmd{oasymbol} if the option \opt{tikzsymbol} is not used.
...
\marginwarn The package \hypersty{biblatex-ext-oa-doiapi} and thus this option
...
\hypersty{biblatex-ext-oa-tikzsymbols} support package.
The optional argument \prm{symbol} has the same meaning as the possible values
for the \hyperopt{tikzsymbol} package option.

The command can only be used in the preamble.
It may only be used once and only if \hypersty{biblatex-ext-oa-tikzsymbols} has
not been loaded before.

This command is related to \hypercmd{DefineTikZOASymbol} of
\hypersty{biblatex-ext-oa-tikzsymbols} (see \cref{sec:oa-symbol}).
...
\hyperopt{tikzsymbol} option of \sty{biblatex-ext-oa}.
\end{ltxsyntax}
...
\item[doiapi] Use API queries from the \sty{biblatex-ext-oa-doiapi} package
  ...
  \marginwarn This option requires that the \hyperopt{doiapi} package option be
...
\label{exltd@itm@biblatex-ext-oa-tikzsymbols}
...
\hypersty[tikzsymbol]{biblatex-ext-oa}.
...
Apart from \hypercmd{oasymbol} the package only defines one other user-level
command.
\begin{ltxsyntax}
...
\begin{release}{0.6a}{2019-01-04}
  \item Minor improvements for \hypersty{biblatex-ext-oa}\see{sec:oasymb}
\end{release}
