...
\exltd@buildhypercmd{cmd,opt,bibmacro,len,sty,bibfield,bibfieldformat}
...
\hypersty[ext-authoryear]{authoryear} and
\hypersty[ext-authortitle]{authortitle} family.
It can not be used with citation styles of the \hypersty[ext-verbose]{verbose}
family, where
...
\item[label] Show the citation label as the label of a list similar to the
             \hypersty[ext-numeric]{numeric} or
             \hypersty[ext-alphabetic]{alphabetic} styles.
\end{valuelist}
...
labels in a \hypersty[ext-numeric]{numeric} bibliography.
...
\optval{inner} you can use \cmd{DeclareOuterCiteDelim} or
\cmd{DeclareInnerCiteDelim} and friends to define the opening and closing
...
\optval{env} value, no bibliography will be printed.}
...
\opt{openaccess} is \optval{auto} and that useful default settings are applied
for semi-automatic open access detection.
...
\optitem[false]{tikzsymbol}{\optval{true}, \optval{false},
  \prm{symbol identifier}}
...
The values \optval{true} and \optval{false} are provided for convenience.
\optval{false} does not load the package at all, \optval{true} is like giving
the value \optval{plos}.
...
Whether or not to load the support package \sty{biblatex-ext-oa-doiapi}
...
\marginwarn The package \sty{biblatex-ext-oa-doiapi} and thus this option can
...
\sty{biblatex-ext-oa-tikzsymbols} support package.
...
It may only be used once and only if \sty{biblatex-ext-oa-tikzsymbols} has
not been loaded before.

This command is related to \hypercmd{DefineTikZOASymbol} of
\sty{biblatex-ext-oa-tikzsymbols} (see \secref{sec:oa-symbol}).
...
options are set to \optval{true}.
...
\marginwarn
With \opt{enable} set to \optval{true} the package will furtermore patch
the bibliography macro \bibmacro{begentry} to execute the bibliography macro
\bibmacro{oamark}.
If additionally \opt{doiapi} is \optval{true}, the bibliography macro
...
\begin{optionlist}
\optitem[auto/none]{openaccess}{\optval{auto}, \optval{doiapi},
  \optval{forcedoiapi}, \optval{true}, \optval{false}, \optval{none},
  \prm{open access field}}

This option controls how the best open access URL is detected.
The initial value is \optval{none} when the package is loaded with the
\opt{enable} option set to \optval{false}, otherwise~-- and by default~-- it is
\optval{auto}.
...
\item[doiapi] Use API queries from the \sty{biblatex-ext-oa-doiapi} package
  ...
  to \optval{true} and only works with \lualatex.
...
\sty{biblatex-oa-doiapi}, which can be loaded via the \hyperopt{doiapi} option
...
load-time option.
\begin{optionlist}
\optitem{symbol}{\optval{plos}, \optval{oanet}}
...
This command redefines \hypercmd{oasymbol} to point to either the PLOS version
or the open-access.net version of the logo.
\end{ltxsyntax}
