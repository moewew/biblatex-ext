...
overlay={%
  ...
  \draw[highlight2, line width=.24mm,|-|] (frame.west)++
    (5.5mm+\introcitewidth,-.98mm) -- node [at start, above=2pt]
...
\begin{bibexample}[enhanced, title={\kvopt{introcite}{label} with
  $\len{introcitewidth}=0$ and $\len{introcitesep}=\len{bibhang}$}]
...
\item Rework lengths for for \kvopt{introcite}{label}.
\end{release}
