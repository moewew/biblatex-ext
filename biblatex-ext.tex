...
This bundle is maintained by Moritz Wemheuer (\textcopyright 2017--2026).
...
For technical reasons the citation label is not produced with a real
cite command.
The user-definable parts of the citation output are controlled by the
bibmacro~\bibmacro{bbx:xrefcite}.
\begin{keymarglistbook}
\keyitem[bbx:xrefcite]
This bibmacro typesets the \enquote{citation} to the parent entry.
With many citation styles it will just call the~\bibmacro{cite}
bibmacro,
but in more complex styles it may also call additional initialisation
or clean-up code.
\end{keymarglistbook}
...
Even though the citation produced by this option is not implemented
via a citation command,
the output can be configured as if it were
produced by a citation command called
\cmd{bbx@xrefcite},
which by default is set up to inherit most styling from \cmd{cite}.
...
\bibmacro{bbx:xrefcite} that emulates the output of \cmd{textcite}.
...
\bibfieldformatitem{biblabeltitle}
  ...
  The default settings mirror the default behaviour of the
  \bibfieldformat{title} field.
  \begin{bibexample}
...
\enquote{\cmd{bbx@xrefcite}}%
  \footnote{Not implemented as a citation command, but can be
    customised as if it were.}
& \cmd{mkouterbbx@xrefcitedelims} &
...
\begin{valuelist}
  ...
  \item[l3draw] Load \hypersty{biblatex-ext-oasymb-l3draw} to provide a
    ...
    \begin{remindbox}
      ...
      You need at least version \texttt{2026-04-28} of \sty{l3draw}.
    \end{remindbox}
...
and \sty{xsavebox}\fnurl{https://ctan.org/pkg/xsavebox} by Alexander Grahn.
...
\sty{tikz}\fnurl{https://ctan.org/pkg/pgf} by Till Tantau,
its dependencies
and \sty{xsavebox}\fnurl{https://ctan.org/pkg/xsavebox} by Alexander Grahn.
...
The code requires version 2026-04-28 or above
of~\sty{l3draw}.
The package also loads
\sty{xsavebox}\fnurl{https://ctan.org/pkg/xsavebox} by Alexander Grahn.
...
\begin{release}{<version>}{<date>}
  \item Fixed vertical alignment of open access symbol in some
    situations.%
    \see{sec:oa:symbol}
\end{release}
\begin{release}{0.21}{2026-05-19}
  \item Removed uses of \cmd{ifthenelse} in favour of \sty{etoolbox}
    commands.
  \item Synced \hypersty{biblatex-ext-oasymb-l3draw}
    with \sty{l3draw} changes.
    Version \texttt{2026-04-28} or above is now required.%
    \see{sec:oa:symbol}
\end{release}
\begin{release}{0.20}{2025-06-24}
  \item Use \sty{xsavebox} in order to typeset open access symbols
  (in \hypersty{biblatex-ext-oasymb-pict2e},
   \hypersty{biblatex-ext-oasymb-tikz},
   and \hypersty{biblatex-ext-oasymb-l3draw}).\see{sec:oa:symbol}
\end{release}
\begin{release}{0.19}{2025-03-31}
  \item Synced \hypercmd{printbibtabular} implementation with
    \cmd{@doendpe} \sty{biblatex}~3.20 changes.
  \item Added \cmd{leavevmode} to \hypersty{biblatex-ext-tabular}'s
    cell macros (\hypercmd{plain}, \hypercmd{plainlang},
    \hypercmd{anchor}, \hypercmd{anchorlang}, \hypercmd{driver}).
\end{release}
